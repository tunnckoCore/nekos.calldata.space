...
function _getBgClass(color: NekoColor) {
  return colors[color].bg;
}

function _getColorVar(color: NekoColor): string {
...
function NekoCard({ neko }: { neko: NekoItem }) {
  return (
    ...
    >
      ...
      {/* Ground */}
      <div
        className={`absolute bottom-0 left-0 h-[35px] w-full ${neko.groundColor.bg}`}
      />
    </div>
  );
}
...
function App() {
  ...
  const generateRandomNeko = useCallback((): NekoItem => {
    ...
    const groundAsDarkerBG = Math.random() < 0.2;

    const getRandomCursor = () =>
      allCursors[Math.floor(Math.random() * allCursors.length)];

    const bg = getRandomColor();
    const ground = groundAsDarkerBG ? bg : getRandomColor();
  ...
  useEffect(() => {
    const initialNekos = Array.from({ length: 48 }, () => generateRandomNeko());
    setNekos(initialNekos);
  }, [generateRandomNeko]);

  const addMoreNekos = () => {
    const newNekos = Array.from({ length: 48 }, () => generateRandomNeko());
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-linear-to-br from-gray-900 to-gray-800">
      <div className="flex-1 overflow-auto p-8">
        ...
        <div className="flex w-full flex-wrap items-center justify-center gap-[4px]">
          {nekos.map((neko) => (
